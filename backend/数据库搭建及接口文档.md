...
- **请求参数说明**:
  | 参数名 | 类型 | 必填 | 描述 |
  | :--- | :--- | :--- | :--- |
  | `administratorname` | String | 是 | 管理员用户名 |
...
##### 3.4.1.1 获取 Markdown 文件列表
- **接口路径**: `/api/user/markdown`
- **请求方法**: GET
- **权限要求**: 需要登录
- **请求参数**:
  | 参数名 | 类型 | 必填 | 描述 |
    ...
    "items": [
      {
        "id": "1",
        "fileName": "示例文档.md",
        "originalName": "示例文档.md",
        "ossUrl": "https://example.com/markdown/file.md",
        "createdAt": "2026-02-19T12:00:00",
        "updatedAt": "2026-02-19T12:00:00"
      }
    ]
...
##### 3.4.1.2 获取 Markdown 文件详情
- **接口路径**: `/api/user/markdown/{id}`
- **请求方法**: GET
- **权限要求**: 需要登录
- **路径参数**:
  | 参数名 | 类型 | 必填 | 描述 |
  | :--- | :--- | :--- | :--- |
  | `id` | String | 是 | Markdown 文件ID |
...
```json
{
  "code": 200,
  "message": "success",
  "data": "https://example.com/markdown/file.md"
}
```

#### 3.4.2 管理员接口（需要登录）

##### 3.4.2.1 上传 Markdown 文件
- **接口路径**: `/api/admin/markdown`
- **请求方法**: POST
- **权限要求**: 需要管理员登录
- **请求参数**:
  | 参数名 | 类型 | 必填 | 描述 |
  | :--- | :--- | :--- | :--- |
  | `file` | MultipartFile | 是 | Markdown 文件 |
  | `images` | MultipartFile[] | 否 | 图片文件数组 |
...
##### 3.4.2.3 获取 Markdown 文件详情
- **接口路径**: `/api/admin/markdown/{id}`
- **请求方法**: GET
- **权限要求**: 需要管理员登录
- **路径参数**:
  | 参数名 | 类型 | 必填 | 描述 |
...
- **请求参数**:
  | 参数名 | 类型 | 必填 | 描述 |
  | :--- | :--- | :--- | :--- |
  | `ids` | List<String> | 是 | Markdown 文件ID列表，多个ID用逗号分隔 |

- **请求示例**:
  `/api/admin/markdown?ids=1,2,3`
...
#### 4.2.1 管理员表 (administrators)
```sql
CREATE TABLE `administrators` (
  `id` VARCHAR(36) NOT NULL PRIMARY KEY COMMENT '管理员ID',
  `administratorname` VARCHAR(50) NOT NULL UNIQUE COMMENT '管理员用户名',
...
#### 4.2.3 管理员登录DTO (AdministratorLoginDTO)
```java
import lombok.Data;
...
#### 4.3.1 Markdown 文件表 (markdown_files)
```sql
CREATE TABLE `markdown_files` (
  `id` VARCHAR(36) NOT NULL COMMENT  '主键ID',
  `file_name` VARCHAR(255) NOT NULL COMMENT '文件名',
  `original_name` VARCHAR(255) NOT NULL COMMENT '原始文件名',
  `oss_url` VARCHAR(512) NOT NULL COMMENT 'OSS文件URL',
  `created_at` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  `updated_at` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
  PRIMARY KEY (`id`),
  KEY `idx_oss_url` (`oss_url`),
  KEY `idx_created_at` (`created_at`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='Markdown文件表';
```

#### 4.3.2 Markdown 图片表 (markdown_images)
```sql
CREATE TABLE `markdown_images` (
  `id` VARCHAR(36) NOT NULL PRIMARY KEY COMMENT '主键ID',
  `markdown_id` VARCHAR(36) NOT NULL COMMENT '所属Markdown文件ID',
  `original_path` VARCHAR(512) NOT NULL COMMENT '原始图片路径',
  `oss_url` VARCHAR(512) NOT NULL COMMENT 'OSS图片URL',
  `created_at` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  KEY `idx_markdown_id` (`markdown_id`),
  KEY `idx_oss_url` (`oss_url`),
  CONSTRAINT `fk_markdown_image` FOREIGN KEY (`markdown_id`) REFERENCES `markdown_files` (`id`) ON DELETE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='Markdown图片表';
```

### 4.4 Markdown Java 类设计

#### 4.4.1 Markdown 文件实体类 (MarkdownFile)
```java
import lombok.Data;
import java.sql.Timestamp;
...
#### 4.4.2 Markdown 图片实体类 (MarkdownImage)
```java
import lombok.Data;
import java.sql.Timestamp;
...
7. **"记住我"功能实现**:
   - **登录时**: 传递`rememberMe`参数，根据用户选择决定是否记住登录状态
   - **Token存储**:
     - 当`rememberMe=true`时，使用`localStorage`存储token，实现持久化存储
     - 当`rememberMe=false`时，使用`sessionStorage`存储token，实现会话级存储
   - **Token获取**: 优先从`sessionStorage`获取，若不存在则从`localStorage`获取
   - **请求拦截器**: 在Axios请求拦截器中添加token到请求头
   - **登出时**: 同时清除`localStorage`和`sessionStorage`中的token
