...
### 1.6 项目图片表 (project_images)
```sql
CREATE TABLE `project_images` (
  `id` VARCHAR(36) NOT NULL PRIMARY KEY COMMENT '图片ID',
  `project_id` VARCHAR(36) NOT NULL COMMENT '项目ID，与projects表id关联',
...
public class ProjectDetailVO {
    ...
    private String title;
    private String description;
    private List<String> images; // 项目图片URL列表
    private String category;
    private List<String> tags;
    private Timestamp createdAt;
    private Timestamp updatedAt;
...
- **title**: 项目标题，支持中英文
- **description**: 项目描述
- **images**: 项目图片URL列表，使用云服务存储，存储在project_images表中
- **category**: 项目分类
- **tags**: 项目标签，多个标签用逗号分隔
- **thoughts**: 摄影项目的思路&感受
...
4. **标签处理**: 前端将标签数组直接传递给后端
5. **图片处理**: 前端直接将图片URL数组传递给后端，后端会自动将其存储到project_images表中
